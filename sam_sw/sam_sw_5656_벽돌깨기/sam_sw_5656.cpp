...
// 외않되?
// vector<pair<int, int> > direction2;
// direction2.push_back(make_pair(0,1));
// direction2.push_back(make_pair(1,0));
// direction2.push_back(make_pair(0,-1));
// direction2.push_back(make_pair(-1,0)); // 동 남 서 북
...
void shoot(int block_col_cord, vector< vector<int> > & blocks){
    for(int row_i = 0; row_i<H; row_i++){
        if(blocks[row_i][block_col_cord]!=0){ // 0이 아닌 첫번째 블록 찾기
            bool already_explode[MAX_H][MAX_W]; // visited 같은 것. 이미 폭발했으면 걔는 지나치고 다음걸 확인하자.
    ...
    while(!que.empty()){
        ...
        if(que_elem.shoot_num<N){
            ...
            for(int c=0; c<W; c++){ // column 크기만큼 돌면서 체크
                ...
                // !!! 홀.... call by reference 안해주면 아예 값이 바뀌지를 않네... 
