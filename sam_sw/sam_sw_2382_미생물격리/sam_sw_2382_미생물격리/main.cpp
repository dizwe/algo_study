...
void simulate(){
    for (int t=1; t<=TIME;t++){
        ...
        for(pair<int, int> one_cell:cell_list){
            ...
            vector< struct cell_info > &this_cell_vector =cell_map[one_cell.first][one_cell.second];
            pair<int, int> next_cord = next_pair(one_cell,this_cell.cell_dir);
            // 있던 애는 빼고 전체는 지우면 안됨!(혹시 뒤에 들어왔던 애가 있을수도 있으니까)
            // erase하면
            this_cell_vector.erase(this_cell_vector.begin());
        ...
        for(pair<int, int> one_cell:next_cell_list){
            ...
            if(is_inside_medicine(one_cell)==true){
                this_cell_vector[0].cell_organ_num /= 2;
                // reference로 좀 더 간단하게 바꿀 방법?
                if(this_cell_vector[0].cell_dir%2==1){
                    this_cell_vector[0].cell_dir ++;
                }else if(this_cell_vector[0].cell_dir%2==0){
                    this_cell_vector[0].cell_dir --;
                }
            }
            // 아니면 여러개 겹치는데 있는지
            ...
            if(this_cell_vector.size()>1){
                int max_num = 0;
                int total_num = 0;
                int next_dir =0;
                for(cell_info will_unite_cell:this_cell_vector){
                ...
                // 하나로 합치기
                this_cell_vector.clear();
                this_cell_vector.emplace_back(cell_info{
